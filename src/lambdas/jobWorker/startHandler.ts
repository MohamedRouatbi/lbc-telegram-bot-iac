...
import { signS3Url, getWelcomeVideoKey, getTTSAudioKey } from '../../lib/s3Signer';
...
async function executeStateAction(
  ...
  switch (state) {
    case 'WELCOME_VIDEO_SENT': {
      ...
      const videoKey = getWelcomeVideoKey(lang);
      const videoUrl = await signS3Url({
      ...
      await sendMessage({
        chatId,
        text: welcomeText,
        buttons: [[{ text: '▶️ Watch Welcome Video', url: videoUrl }]],
      });
    ...
    case 'TTS_SENT': {
      ...
      // Sign the TTS URL
      const audioKey = getTTSAudioKey(s3Key);
      const audioUrl = await signS3Url({
        bucketName: TTS_BUCKET,
        objectKey: audioKey,
        ttlSeconds: 600, // 10 minutes
      });
      ...
      await sendMessage({
        chatId,
        text: ttsText,
        buttons: [[{ text: '🔊 Play Greeting', url: audioUrl }]],
      });
